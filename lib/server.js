...
const { getExplorersStacks } = require("./services/ExplorerService");
const app = express();
...
app.get("/v1/explorers/stack/:mission", (request, response) => {
    const mission = request.params.mission
    const explorersStacks = ExplorerController.getExplorersStacks(mission);
    response.json({stack : mission, explorers: explorersStacks})
});
