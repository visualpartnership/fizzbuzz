...
app.get("/v1/explorers/stack/:name", (request, response) => {
    const stackName = request.params.name;
    const explorersWithGivenStack = ExplorerController.getExplorersWithStack(stackName);
    response.json(explorersWithGivenStack);
});
